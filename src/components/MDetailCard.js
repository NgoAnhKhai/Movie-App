...
function MDetailCard({ movieDetail, loading }) {
  ...
  const addFavMovie = (title, poster, voteA, voteC, id) => {
    if (!id) {
      setmovieError("Invalid movie details");
      return;
    }

    let list = JSON.parse(localStorage.getItem("fav"));
    if (list) {
      const movieExists = list.find((item) => item.id === id);
      if (movieExists) {
        setmovieError("You already have this item!");
      } else {
        list.push({
          id: id,
                ...
                <IconButton
                  onClick={() => {
                    if (movieDetail && movieDetail.id) {
                      ...
                      setmovieError("Movie details are missing");
                    }
                  }}
                  size="large"
